...
### 전환 제외 앱 위의 Spotlight

단위 테스트는 IMK가 활성화를 알린 뒤의 판정만 다룬다. 실제 순서, 즉 Spotlight 입력칸의 `activateServer`가 첫 키보다 먼저 PriType 메인 스레드에 닿는지는 실기기에서만 확인할 수 있다. 합성 키로는 확인할 수 없다(위 절). 원격 데스크톱 클라이언트(예: Windows App)를 제외 목록에 넣고 앞에 둔 채 확인한다.

1. ⌘Space로 Spotlight를 열자마자, 다른 키보다 **전환키를 먼저** 누른다. Spotlight의 입력 모드가 바뀌어야 한다. 원격 세션에는 아무것도 가지 않아야 한다.
2. Spotlight를 연 직후 한자키를 먼저 누른다. 원격 세션에 우측 ⌥가 가지 않아야 한다.
3. Esc로 닫자마자 전환키를 누른다. 원격 세션의 입력기가 전환키를 받아야 하고, PriType 모드는 바뀌지 않아야 한다.
4. Spotlight에서 전환키를 누른 채 Esc로 닫은 뒤 전환키를 뗀다. 원격 세션에서 ⌘가 눌린 채 남지 않아야 한다. 다음 전환키는 원격 세션으로 가야 한다.
5. 반대로, 원격 세션에서 전환키를 누른 채 ⌘Space를 누르고 뗀다. Spotlight가 열리든 원격 세션이 가로채든, PriType 모드는 바뀌지 않아야 한다.

macOS 27에서 Spotlight 입력칸의 IMK 클라이언트 번들 ID는 `com.apple.Spotlight`가 아니라 `com.apple.campo`다.

2026-09-26 이 Mac에서 합성 키로 지연을 쓸어 본 결과(미리보기를 제외 앱으로):

- 앱을 클릭해 옮긴 직후의 전환키는 0ms부터 맞았다. 활성화 알림이 키보다 먼저 온다.
- Esc로 Spotlight를 닫은 뒤의 전환키는 10ms부터 맞았다. AX상 포커스는 약 23ms에 넘어간다.
- ⌘Space로 연 뒤 **전환키가 첫 키이면** 약 300ms까지는 틀렸다. AX상 포커스는 약 80ms에 넘어가지만, Spotlight 입력칸의 IMK 활성화는 250~350ms 뒤에야 온다. 그 사이의 전환키는 Spotlight로 그냥 넘어가고(원격 세션으로 가지는 않는다) 전환이 일어나지 않는다. 다시 누르면 된다.
- 글자를 먼저 치면 그 키가 활성화를 앞당겨, 100ms에 친 글자 다음의 전환키도 맞았다.

1이나 3이 첫 키에서만 틀리고 두 번째 키부터 맞으면, IMK의 활성화나 비활성화가 첫 키보다 늦게 온 것이다. 1은 디버그 로그에서 `Activated for client: com.apple.campo`의 시각과 탭의 `Toggle key DOWN` 시각을 비교해 확인한다.
