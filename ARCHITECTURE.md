...
이 문서는 현재 코드가 실제로 하는 일을 설명한다. 파일 이름은 `Sources/PriTypeCore/` 기준이다. 지금의 구조가 왜 이렇게 됐는지와 폐기된 대안은 [Docs/History.md](Docs/History.md)에 있다.
...
1. `--abc-layout-status` 인자로 실행되면 ABC 자판 상태만 출력하고 종료한다(`ABCLayoutStatusProbe`, 아래 "입력 소스 관리" 참고).
   `--authorize-update-install <스테이징 디렉터리>`로 실행되면 거기 놓인 업데이트를 검증한 뒤 관리자 암호만 묻고 종료 코드로 답한다(아래 "업데이트" 참고).
2. `IMKServer`를 만든다.
...
5. 키 모니터를 시작한다(`KeyMonitors.start`, 설정 창의 권한 버튼도 같은 함수를 부른다).
   ...
   - `RightCommandSuppressor`(CGEventTap)를 시작한다. 탭은 만들어지는 순간 켜져 있으므로 바로 끄고, 전용 스레드가 소스를 붙인 뒤에 켠다. IOKit 대체 경로는 탭이 준비된 뒤, 켜기 직전에 멈춘다. 그래서 그 사이의 누름은 두 모니터 중 정확히 하나만 본다(둘 다 놓치지도, 둘 다 전환하지도 않는다). 이때 `hidd`와의 블로킹 IPC가 탭 잠금 안에서 돌지만, 탭이 아직 꺼져 있어 그 잠금을 기다리는 키 입력은 없다. 실패하거나 나중에 탭이 시간 초과로 반복해서 꺼지면 `IOKitManager`로 넘기고, 60초 뒤 탭을 다시 시도한다. IOKit 경로는 입력 모니터링 권한을 확인하고, 없으면 요청한 뒤 허용될 때까지 2초마다 확인한다.
...
- 영문 모드: 조합 중인 글자가 있으면 확정하고 `false`를 돌려준다. 키 처리는 앱이 한다. 예외는 더블스페이스 마침표다. macOS는 이것을 입력 소스에서 처리하므로 서드파티 입력기 뒤에서는 어느 앱도 하지 않는다. 그래서 빠른 두 번째 공백일 때만 커서 앞 두 글자를 앱에서 읽어, 한글 모드와 같은 확인 경로로 바꾼다. 첫 공백은 앱이 넣는다.
- 한자 후보창이 떠 있으면 키를 후보창에 먼저 넘긴다.
- ⌘·⌃·⌥가 눌린 키: 조합을 확정하고 앱으로 넘긴다(단축키). ⌘는 그보다 먼저, 누르는 순간 탭이 확정을 요청한다(`InputModeCoordinator.requestShortcutCommit`). 앞서 친 키가 아직 IMK에 오는 중이면 그 키를 기다리고, 아니면 곧바로 확정한다. Chromium·Electron은 조합 중에 도착한 키를 IME가 처리한 키(`VKEY_PROCESSKEY`)로 페이지에 넘기고, 입력기가 편집 명령을 남긴 경우(⌘←)에만 원래 키를 다시 보낸다. 단축키의 키에서 확정하면 ⌘A·⌘Z·⌘Enter가 사라진다. 한자 후보창이 떠 있으면 확정하지 않는다.
...
앱을 막 바꾼 직후에는 IMK가 첫 키를 다른 컨트롤러에 주고 몇 ms 뒤 그 컨트롤러를 거두어 같은 앱의 다른 컨트롤러를 활성화하기도 한다(실측: KakaoTalk 4ms, TextEdit 16ms). 거둔 클라이언트에 확정한 글자는 어느 입력칸에도 들어가지 않는다(카톡 → ㅏ톡). 그래서 컨트롤러가 엔진을 넘겨받은 지(또는 새 세션을 연 지) `churnWindow`(50ms)도 안 되어 떠나는 조합 표시는 확정하지 않고, 음절을 공유 엔진에서 꺼내 넘겨 둔다. 엔진이 비어 있으므로 그사이 다른 세션의 확정 경로는 그 음절을 건드리지 못한다. 같은 앱의 세션이 오면 음절을 엔진에 되돌리고 표시를 다시 해서 이어 조합한다. 그사이 모드가 영어로 바뀌었으면(전환키, 또는 macOS가 활성화 전에 영어를 선택) 표시하지 않고 그 입력칸에 확정한다. 다른 앱에 키가 입력되거나 아무 세션도 이어받지 않은 채 대기(보류된 비활성화와 같은 0.3초)가 끝나면 전처럼 원래 클라이언트에 확정한다. 다른 앱이 활성화되는 것만으로는 판정하지 않는다. KakaoTalk에서 TextEdit으로 옮길 때 IMK는 TextEdit의 입력칸에 앞서 KakaoTalk을 한 번 더 활성화하고, 그 뒤 입력칸의 다음 키는 50ms보다 늦게 올 수 있다. 직접 삽입은 글자가 이미 문서에 있으므로 넘기지 않는다. 사람은 입력칸을 켜고 50ms 안에 글자를 치고 떠날 수 없으므로 평소 동작은 바뀌지 않는다.
...
`RightCommandSuppressor`가 전용 스레드의 `CGEventTap`으로 전환키(기본 우측 ⌘)와 한자키(기본 우측 Option)를 감지한다. 설정 창의 키 녹음으로 수정키 하나, 일반 키, 조합 키 모두 등록할 수 있다. 조합 키는 ⌘⌃⌥⇧가 정확히 같을 때만 동작한다(⌃Space 바인딩이 ⌃⌥Space·⌃⌘Space 같은 macOS 단축키를 가로채지 않게). Caps Lock과 Fn은 보지 않는다.

입력칸에 포커스가 없을 때(Finder, 입력칸이 선택되지 않은 웹 페이지)도 전환한다. 탭이 이미 키를 삼켰으므로, 조합기의 모드만 바꾸고 macOS에 알린다. 로마자 자판 지정은 다음 활성화가 한다. 단, PriType이 선택된 입력 소스일 때만이다. 사용자가 다른 입력 소스(ABC, 일본어 등)로 바꿔도 컨트롤러가 사라지는 것은 같아서, 확인하지 않으면 모드 통보가 PriType을 다시 선택해 버린다.
...
- 전환 제외 앱(`ToggleExclusionPolicy`): 원격 데스크톱·가상 머신처럼 자체 입력기를 쓰는 앱이 키를 받을 때는 전환키와 한자키를 가로채지 않고 그대로 넘긴다.
  - 키를 받는 앱은 포커스 소유자를 먼저 본다. IMK가 PriType을 활성화한 입력칸의 앱이다(`focusDidMove`, 세션이 이미 가진 번들 ID라 호스트 호출이 없다). 활성화보다 키를 먼저 보내는 호스트가 있어서, `handle()`도 세션을 정한 뒤 같은 통지를 한다. 값이 같으면 비교 한 번으로 끝난다. 그 컨트롤러가 비활성화되면 지운다. 늦게 온 이전 입력칸의 비활성화는 새 소유자를 지우지 못한다. 소유자가 없으면 앞에 있는 앱으로 판정한다. 입력칸이 없는 원격 세션, 번들 ID를 주지 않는 클라이언트, 다른 입력 소스가 선택된 경우가 여기에 해당한다. 앱이 활성화되면 소유자를 잊는다.
...
| `IOKitManager` (IOHIDManager, 대체) | 입력 모니터링 | 탭 생성에 실패하거나, 탭이 시간 초과로 60초 이내 간격으로 세 번 연달아 꺼지면(`EventTapFailureTracker`) 넘겨받는다. 사용자 입력으로 꺼진 것(`tapDisabledByUserInput`)은 세지 않고 늘 다시 켠다. 탭 하나가 넘기는 일은 한 번뿐이며, 60초 뒤 탭을 다시 시작해(`KeyMonitors`) 되면 IOKit은 멈추고, 안 되면 다시 넘긴다. 시간 초과는 기기 전체가 멈칫한 탓이지(콜백은 1μs 미만) 탭의 탓이 아니므로, 대체 경로는 그 사이를 잇는 다리일 뿐이다. 규칙의 시각은 HID 값 자신의 타임스탬프(단조 시계)다. 키를 볼 수만 있고 막을 수는 없으므로, 누르는 순간 모드에서는 우측 ⌘ + C가 전환과 복사를 함께 한다. 규칙은 `HIDShortcutState`에 있다 |
...
누구든 PriType을 이 인자로 실행할 수 있으므로 자식은 호출한 쪽이 건넨 것을 믿지 않는다. 받는 것은 스테이징 디렉터리 하나뿐이고, 부모가 내려받아 둔 패키지·`update.json`·`update.json.sig`를 스스로 다시 검증한다. 릴리스 키의 서명, 실행 중인 버전보다 새 버전인지, 패키지의 크기와 SHA-256이 모두 맞아야 암호를 묻는다. 그래서 이 진입점으로 할 수 있는 최대치는 진짜 PriType 업데이트를 제안하는 것이다. root는 패키지를 자기만 쓸 수 있는 곳에 복사한 뒤 해시를 한 번 더 확인하므로, 검증 뒤에 파일을 바꿔치기해도 설치되지 않는다.
...
탭 스레드가 닿는 타입이 그 여섯뿐인 것은 아니다. `InputModeCoordinator`와 `HanjaCandidateWindow`도 탭 콜백에서 들어오고 둘 다 아직 `@unchecked Sendable`이다. 다만 탭이 건드리는 것은 각각 `pendingActions`와 `pageCandidatesState`, 즉 **자체 잠금이 걸린 대기열 하나씩뿐**이고, 나머지 필드는 `Thread.isMainThread` 분기 뒤 메인에서만 쓴다. 패키지에는 `@unchecked`가 아직 열둘 더 있고(main에서는 스물둘이었다), 그 나머지는 IMK가 메인에서만 부른다는 전제로 사는 타입들과 로그 큐다 — 별개 문제다. 상태를 여기에 넣은 타입은 다른 가변 저장 프로퍼티가 없으므로 `Sendable`을 **검사받아서** 만족한다 — 약속이 아니라 컴파일러가 확인한다. `OSAllocatedUnfairLock`이 검사받는 대안이지만 상태가 `CFMachPort`·`CFRunLoopSource`·`CFRunLoop`·`IOHIDManager`라 쓸 수 없다. 재귀인 이유는 탭 콜백이 이벤트 하나를 처리하는 도중 `stop()`을 부르기 때문이고, 상태를 구조체가 아니라 클래스로 두는 이유도 같다(재진입한 `withLock`이 같은 객체를 준다).
...
`startAndWait`는 탭 스레드의 런루프가 소스를 받을 때까지 최대 2초 세마포어를 기다리는데, 그것도 이 잠금을 쥔 채다(`start()` 경로, 메인).

**지켜야 할 규칙은 이 전부가 잎(leaf)이라는 것이다.** 어느 것도 `RightCommandSuppressor`의 잠금을 되잡지 않으므로 순환이 없다. `HanjaCandidateWindow`와 `InputModeCoordinator`는 잠긴 대기열에 넣기만 하고 실제 작업은 메인 큐로 넘기므로 잠금 안에서 돌지 않는다. 반대 방향이 생길 뻔한 곳이 실제로 있었다: `IOKitManager.handleKeyboardEvent`는 제외 정책·녹화 여부·설정값을 읽는데, 이것들을 **자기 잠금 밖에서** 먼저 해소한다. 잠금 안에서 읽었다면 `IOKit → RightCommandSuppressor` 간선이 생겨 위의 `RightCommandSuppressor → IOKit`과 맞물렸을 것이다. 새 코드가 이 잎들 중 하나에서 다른 싱글턴을 부르면 시스템 전체의 타이핑이 멈춘다.
...
Swift Testing 기반, 479개 테스트와 71개 Suite(2026-09-20 기준). Command Line Tools에는 Testing 모듈이 없으므로 Xcode 툴체인이 필요하다.
...
│   ├── PriTypeDeviceCheck/      # 실기기 검증 검사들
│   ├── PriTypeDeviceCheckCLI/   # pritype-device-check
│   └── PriTypeBenchmark/
