...
- 전환 시점(`ToggleTrigger`, 전환키가 수정키 하나일 때만)
  - 누르는 순간(`press`, 기본): 키를 삼키고 즉시 전환한다. 누르고 있는 동안 다른 키에서 그 수정키를 떼어 내므로 우측 ⌘ + C는 c를 입력한다. 한자키(수정키 하나일 때)도 누를 때 삼키므로 같은 방식으로 떼어 낸다. 떼어 내기는 후보창 키 라우팅보다 먼저 하므로, 한자키를 떼기 전에 친 숫자도 후보를 고른다.
