...
앱을 막 바꾼 직후에는 IMK가 첫 키를 다른 컨트롤러에 주고 몇 ms 뒤 그 컨트롤러를 거두어 같은 앱의 다른 컨트롤러를 활성화하기도 한다(실측: KakaoTalk 4ms, TextEdit 16ms). 거둔 클라이언트에 확정한 글자는 어느 입력칸에도 들어가지 않는다(카톡 → ㅏ톡). 그래서 컨트롤러가 엔진을 넘겨받은 지(또는 새 세션을 연 지) `churnWindow`(50ms)도 안 되어 떠나는 조합 표시는 확정하지 않고 넘겨 두고, 그 창 안에 이어지는 같은 앱의 세션이 표시를 다시 해서 이어 조합한다. 다른 앱이거나 창이 지났거나 아무 세션도 이어받지 않으면(보류된 비활성화와 같은 대기 뒤) 전처럼 원래 클라이언트에 확정한다. 직접 삽입은 글자가 이미 문서에 있으므로 넘기지 않는다. 사람은 입력칸을 켜고 50ms 안에 글자를 치고 떠날 수 없으므로 평소 동작은 바뀌지 않는다.
