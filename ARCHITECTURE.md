...
5. 키 모니터를 시작한다(`KeyMonitors.start`, 설정 창의 권한 버튼도 같은 함수를 부른다).
   ...
   - `RightCommandSuppressor`(CGEventTap)를 시작한다. 탭은 만들어지는 순간 켜져 있으므로 바로 끄고, 전용 스레드가 소스를 붙인 뒤에 켠다. IOKit 대체 경로는 탭을 만들기 전에(탭 잠금 밖에서) 멈춘다. 실패하거나 나중에 탭이 시간 초과로 반복해서 꺼지면 `IOKitManager`로 넘기고, 60초 뒤 탭을 다시 시도한다. IOKit 경로는 입력 모니터링 권한을 확인하고, 없으면 요청한 뒤 허용될 때까지 2초마다 확인한다.
