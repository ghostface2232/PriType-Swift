...
- 주 화면 왼쪽이나 아래에 놓인 보조 모니터에서 한자 후보창이 커서 옆에 뜨지 않던 문제를 수정했습니다. 커서 좌표 검증이 x, y가 1보다 커야 한다는 조건으로 음수 좌표를 모두 거부했는데, 그런 모니터의 좌표는 원래 음수입니다. 후보창은 이전 위치나 마우스 위치로 밀려났습니다. 이제 쓰레기값은 형태(0, 무한대·NaN, 비정규화 부동소수점)와 어느 화면에도 없는지로만 걸러냅니다.

### 수정 (키보드 배열과 Caps Lock — 2026-09-18)
- 한글 모드에서 키를 현재 라틴 배열이 만든 문자가 아니라 키 위치로 해석합니다. 기존에는 Dvorak, Colemak, AZERTY 같은 배열을 쓰면 자모가 엉뚱하게 입력됐습니다. 두벌식은 US QWERTY 키 위치로 정의되므로, 이제 어떤 배열에서도 같은 키가 같은 자모를 냅니다. 키패드, JIS·ISO 전용 키, 기능 키는 기존처럼 시스템이 만든 문자를 씁니다.
