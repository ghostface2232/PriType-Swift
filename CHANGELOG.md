...
- 한 음절 후보도 여러 음절과 같이, 앱이 알려 주는 커서 앞 글자가 후보의 한글과 같을 때만 바꿉니다. 커서가 어떤 이유로든 옮겨졌다면 선택을 취소합니다. NFD로 저장된 글자 뒤에서 한 음절 후보를 고르면 마지막 자모만 바뀌어 "구國"처럼 깨지던 경우도 이제 취소됩니다. 커서 앞 글자를 알려 주지 않는 앱에서는 기존처럼 확인 없이 바꿉니다.
