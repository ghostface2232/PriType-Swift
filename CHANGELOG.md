...
- 이제 글자 키가 글자가 아닌 문자를 내는 것을 보면, 한국어 모드에서 숫자와 문장부호 키를 US 자판 위치로 읽습니다. 쉼표는 US 쉼표 자리에서 나오는 식으로 빠짐없이 칠 수 있습니다. 판정은 각 글자 키가 마지막으로 친 문자로 하므로, 앱이 ABC 자판으로 바뀌는 등 배열이 달라지면 따라갑니다.
- 독일어·북유럽처럼 글자 키가 모두 글자인 배열은 이전처럼 ö·ü·å 같은 문자와 배열의 문장부호를 그대로 씁니다. 영문 모드는 바뀌지 않았습니다.

### 수정 (포커스를 옮겨도 한자 후보창이 남아 키를 가로챔 — 2026-09-19)
- 한자 후보창이 떠 있는 채로 다른 입력창이나 앱으로 옮겼을 때, IMK가 새 입력창을 먼저 활성화하고 이전 입력창을 나중에 비활성화하면 후보창이 닫히지 않던 문제를 수정했습니다. 늦게 온 비활성화는 이미 조합기의 주인이 아니어서 창을 닫지 않았고, 그동안 이벤트 탭이 새 앱에서 친 Tab, ↑↓, `[` `]`, 숫자를 후보창으로 가져가 입력되지 않았습니다. 이제 다른 입력창이 조합기를 넘겨받는 순간 후보창을 닫습니다.
