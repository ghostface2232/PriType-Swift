...
- 앱이 이 바꾸기를 적용하지 않는 경우(크롬 계열이 커서 위치를 늦게 알려 주거나, 앱이 범위 지정을 무시하는 경우)에는 바로 다음 백스페이스를 앱에 그대로 넘겨, 수정 전과 같이 자모 하나가 지워지게 합니다. 바로 이어진 백스페이스이고 같은 자리에 같은 글자가 그대로일 때만 그렇게 하며, 마우스 클릭이나 다른 키가 사이에 끼면 다시 정상으로 바꿉니다.
- Google Docs는 예외입니다. Docs는 화면을 직접 그리고 입력기에는 빈 입력 영역만 보여 줘서, 커서 위치를 늘 "0번 자리에 길이 1짜리 선택"으로, 문서 길이를 0으로 알려 줍니다. 커서 앞에 무슨 글자가 있는지 알 수 없어 PriType은 물러나고, Docs에서는 예전처럼 자모 단위로 지워집니다.
