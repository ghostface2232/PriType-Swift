...
- 탭이 메인 밖으로 나가면서 "전환키 → 메인으로 이동"과 "다음 글자 → IMK `handle()`"의 순서를 더는 메인 스레드 차단이 보장해 주지 않습니다. 그래서 전환을 키가 눌린 순간 기록하고, `handle()`이 조합 전에 기록된 전환을 먼저 적용하도록 했습니다. 전환 직후 첫 글자는 큐 타이밍과 무관하게 새 모드로 들어갑니다.
